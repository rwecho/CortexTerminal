...
(() => {
  ...
  let sendMessageFunction: any = null;

  function resolveSendMessageFunction() {
    if (window.chrome && window.chrome.webview) {
      // Windows WebView2
      sendMessageFunction = (msg: any) =>
    ...
    if (window.hybridWebViewHost) {
      // Android WebView
      sendMessageFunction = (msg: any) =>
        window.hybridWebViewHost!.sendMessage(msg);
      return sendMessageFunction;
    }
  ...
  function initHybridWebView() {
    ...
    if (
      ...
      w.external.receiveMessage = (message: any) => {
   ...
   */
  function invokeJavaScriptCallbackInDotNet(taskId: any, result: any) {
   ...
   */
  function invokeJavaScriptFailedInDotNet(taskId: any, error: any) {
    let errorObj;
    if (!error) {
      errorObj = {
   ...
   */
  async function invokeDotNet(methodName: any, paramValues: any) {
    const body = {
   ...
   * @returns A promise.
   */
  async function invokeJavaScript(taskId: any, methodName: any, args: any) {
    try {
